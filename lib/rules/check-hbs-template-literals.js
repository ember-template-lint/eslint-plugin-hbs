      ...
      TaggedTemplateExpression: (node) => {
        if (node.tag.name === 'hbs') {
          if (node.quasi.type === 'TemplateLiteral') {
            ...
            const results = getResults({hbs, context});
...
function getResults(options) {
  let runAsync = async ({hbs, context}) => {
    let TemplateLinter;
    function getLinter(context) {
      let config = {}
      let linter;
    ...
    let linter = getLinter(context);
    let results = await linter.verify({source: hbs.toString(), filePath: context.id, moduleId: context.id});

    return results;
  }

  const runSync = forceSync(runAsync);
